...
| `neighbor_weight` | 1.0 | |
| `scale_reg_weight` | **20.0(잠정)** | §8.2 참고 — 09-08 기준 재스윕 진행 중, rw=10이 더 나을 가능성 있음 |
| `w_bits` / `a_bits` | 8 / 8 | |
...
### 8.1 메인 결과 — `scale_reg_weight=20`, 5-seed(0,1,2,4,5; seed3는 OOM으로 소실)

| method | COCO_AP | LVIS_AP | Heval_flip | Top1_flip | UPIR | lost | LVIS_flip | LVIS_lost |
|---|---|---|---|---|---|---|---|---|
| **FP32** | **36.80** | **0.1260** | - | - | - | - | - | - |
| naive | 33.54 | 0.1264 | 12.19% | 0.95% | 0.28% | 432 | 6.48% | 1224 |
| AdaRound | 33.24 | 0.1242 | 11.28% | 0.74% | 0.24% | 349 | 5.67% | 1212 |
| QDrop | 33.30 | 0.1235 | 11.25% | 0.72% | 0.26% | 384 | 5.71% | 1192 |
| BRECQ | 33.30 | 0.1200 | 10.86% | 0.71% | 0.22% | 327 | 5.53% | 1158 |
| **Combined** | **36.09±0.09** | 0.1247±0.0013 | **9.74±0.78%** | 0.73±0.02% | 0.23±0.07% | 369.2±22.0 | **5.32±0.07%** | **1146.6±27.9** |

*원본 로그: `runs/58_official_data/seed{0,1,2,4,5}.log`*

**정직한 요약**:
- COCO_AP: naive 대비 **+2.55**, 5-seed 전부 안정적(±0.09).
- Heval_flip·LVIS_flip: **5/5 seed 전부** BRECQ(이 지표 최선 baseline)를 이김
  — 이 프로젝트에서 masked flip을 이긴 건 이 설계가 처음.
- LVIS_lost: 4/5 seed가 BRECQ보다 낮음, 평균도 이김.
- **LVIS_AP**: naive(0.1264)가 오히려 Combined(0.1247)보다 높음 — **5/5 seed
  전부** naive보다 낮은, 작지만 일관된(-1.3% 상대) 열세. 원인: `s_mult`가
  COCO-80 S/이웃 구조에 맞춰 조정되기 때문에, 그 최적화가 전혀 안 닿는
  LVIS 전용 1100여개 class에는 미세한 잔여 편향으로 남음(§5.2의 재설계로
  "5개 중 최악"에서 "naive 다음"까지 줄었지만 완전히 0은 아님).
- UPIR·lost: baseline 중 최선(AdaRound/BRECQ)에는 못 미치는 중위권.
...
### 8.2 `scale_reg_weight` 재스윕 — 진행 중, 잠정 결과

1-seed(seed0) 스윕 결과 (`scripts/59_rw_sweep_official_data.py`, `runs/59_rw_sweep/rw_sweep_seed0.log`):

| rw | COCO_AP | LVIS_AP | Heval_flip | Top1_flip | UPIR | lost | LVIS_flip | LVIS_lost |
|---|---|---|---|---|---|---|---|---|
| 10 | **36.38** | **0.1259** | 8.57% | 0.71% | **0.27%** | 353 | 5.26% | **1067** |
| 20(현재값) | 36.23 | 0.1224 | 8.55% | 0.73% | 0.29% | 368 | 5.24% | 1199 |
| 30 | 35.99 | 0.1239 | 8.73% | 0.78% | 0.33% | 416 | 5.49% | 1153 |
| 50 | 35.45 | 0.1257 | 8.50% | 0.70% | 0.32% | 367 | 5.33% | 1153 |

rw=10이 rw=20보다 대부분 지표(COCO_AP, LVIS_AP, UPIR, lost, LVIS_lost)에서
나아 보임 — 3-seed(0,1,2,4) 검증이 GPU 5/6/7에서 **진행 중**(`runs/58_official_data/rw10_seed{1,2,4}.log`,
seed0은 위 스윕 값 재사용). **이 문서의 §8.1은 아직 rw=20 기준이며, rw=10
검증이 끝나면 "메인 결과"를 rw=10으로 교체할지 판단 예정.**
