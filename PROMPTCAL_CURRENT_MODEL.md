...
### 8.8 `scale_reg_weight` x `neighbor_weight=0.25` 조합 — 유력 후보 발견 (2026-09-10, 1-seed, **미확정**)

§9에서 짚은 "Combined가 APr(rare)에서 5개 중 최하위" 문제와 §8.1의
"LVIS_AP 잔여 열세"를 동시에 겨냥해서, neighbor_weight=0.25를 고정하고
scale_reg_weight(rw)를 재스윕했다(rw를 올리면 s_mult가 1.0에 더 세게
붙잡혀서 naive/AdaRound의 "안 건드린" 상태에 가까워짐 — §9의 "vocabulary
과적합" 논의 참고).

| method | COCO_AP | LVIS_AP | APr(rare) | Heval_flip | Top1_flip | UPIR | lost | LVIS_flip | LVIS_lost |
|---|---|---|---|---|---|---|---|---|---|
| naive | 33.54 | 0.1264 | 0.0415 | 10.51% | 0.95% | 0.39% | 432 | 6.48% | 1224 |
| AdaRound | 33.24 | 0.1242 | 0.0328 | 9.20% | 0.74% | 0.26% | 349 | 5.67% | 1212 |
| QDrop | 33.30 | 0.1235 | 0.0311 | 9.25% | 0.72% | 0.33% | 384 | 5.71% | 1192 |
| BRECQ | 33.30 | 0.1200 | 0.0319 | 8.99% | 0.71% | 0.33% | 327 | 5.53% | 1158 |
| Combined, nw=0.25·rw=5 | 36.41 | 0.1245 | 0.0378 | 9.36% | 0.82% | 0.25% | 357 | 5.33% | 1168 |
| Combined, nw=0.25·rw=10 | 36.19 | 0.1235 | 0.0366 | 8.51% | 0.72% | 0.27% | 377 | 5.00% | 1074 |
| **Combined, nw=0.25·rw=20** | 35.91 | **0.1256** | **0.0354** | **8.01%** | **0.69%** | **0.30%** | 355 | 5.00% | 1108 |
| Combined, nw=0.25·rw=30 | 35.38 | 0.1210 | 0.0292 | 8.54% | 0.73% | 0.30% | 360 | 5.36% | 1180 |

*원본: `runs/60_hparam_sweep/rw_sweep_nw025_seed0.log`*

**`nw=0.25, rw=20` 조합이 지금까지 나온 후보 중 가장 균형 잡혀 보임**:
- LVIS_AP 0.1256 — naive(0.1264)에 근접, AdaRound(0.1242)·QDrop(0.1235)·BRECQ(0.1200) 전부 넘어섬.
- APr(rare) 0.0354 — 확정값(nw=1,rw=10)의 0.0270(5개 중 최하)에서 크게
  개선돼서 AdaRound(0.0328)·QDrop(0.0311)·BRECQ(0.0319)를 전부 넘어섬
  (naive의 0.0415만 못 넘음). §9에서 짚은 rare-class 문제가 상당히 완화됨.
- **Top1_flip 0.69% — 처음으로 BRECQ(0.71%)를 이김.**
- Heval_flip(8.01%)·UPIR(0.30%)·LVIS_flip(5.00%, 최고 동률)·LVIS_lost(1108)
  전부 BRECQ보다 좋음. **`lost`(355 vs BRECQ 327)만 아직 못 넘음** — 유일하게
  안 풀린 지표.
- 비용은 COCO_AP -0.58(확정값 36.49→35.91) — 그래도 baseline 최고(naive
  33.54) 대비 +2.37로 여전히 압도적.
- rw=30에서는 다시 나빠짐(과도한 정규화로 보임) — rw=20이 국소 최적점.

**해석**: rw를 높여서 s_mult를 naive에 가깝게 되돌리는 것이 실제로
"vocabulary 과적합"을 완화하는 방향으로 작동한다는 가설과 일치하는
결과. nw=0.25 하나만으론 못 풀던 lost/APr 문제를, rw=20과 조합하니
APr은 거의 풀렸고 lost도 남은 유일한 미해결 지표로 좁혀짐.

**다음**: `nw=0.25, rw=20` 근방(rw=15, 25 등) 더 세밀하게 탐색하거나,
바로 6-seed로 정식 검증할지 판단 필요. 아직 1-seed라 seed 노이즈
가능성은 남아있음.

### 8.9 `nw=0.25, rw=20` 6-seed 검증 결과 — **확정값 대체 기각** (2026-09-10)

§8.8의 seed0 결과가 유독 좋았던 건지 확인하려고 6-seed(0~5) 전부 돌렸다
(`runs/58_official_data/nw025_rw20_seed{0..5}.log`, 전부 에러 없이 완료).
**결론부터: seed0은 운 좋은 draw였고, 6-seed 평균으로는 확정값을 대체할
근거가 안 된다.**

| seed | COCO_AP | LVIS_AP | APr | Heval_flip | Top1_flip | UPIR | lost | LVIS_flip | LVIS_lost |
|---|---|---|---|---|---|---|---|---|---|
| 0 | 35.91 | 0.1256 | 0.0354 | 8.01% | 0.69% | 0.30% | 355 | 5.00% | 1108 |
| 1 | 35.84 | 0.1215 | 0.0339 | 9.09% | 0.76% | 0.36% | 397 | 5.32% | 1201 |
| 2 | 35.82 | 0.1246 | 0.0294 | 10.05% | 0.74% | 0.24% | 353 | 5.25% | 1108 |
| 3 | 35.81 | 0.1278 | 0.0394 | 8.14% | 0.76% | 0.29% | 373 | 5.40% | 1113 |
| 4 | 35.67 | 0.1245 | 0.0407 | 10.30% | 0.78% | 0.23% | 383 | 5.38% | 1156 |
| 5 | 35.73 | 0.1228 | 0.0364 | 11.59% | 0.80% | 0.14% | 383 | 5.55% | 1148 |
| **평균(6-seed)** | **35.80** | **0.1245** | **0.0359** | **9.53%** | **0.76%** | **0.26%** | **374** | **5.32%** | **1139** |

**확정값(§8.1, nw=1·rw=10, 6-seed 평균) 대비**:

| method | COCO_AP | LVIS_AP | Heval_flip | Top1_flip | UPIR | lost | LVIS_flip | LVIS_lost |
|---|---|---|---|---|---|---|---|---|
| **확정값** | **36.30** | 0.1240 | **9.39%** | **0.74%** | **0.25%** | **372** | 5.37% | **1122** |
| nw=0.25·rw=20 | 35.80 | **0.1245** | 9.53% | 0.76% | 0.26% | 374 | **5.32%** | 1139 |

COCO_AP는 확정값이 확실히 위(+0.50), Heval_flip/Top1_flip/UPIR/lost/
LVIS_lost도 전부 확정값이 근소하게 낫다. LVIS_AP(+0.0005)·LVIS_flip
(+0.05pp)만 근소하게 이 후보가 낫지만, 둘 다 각 지표의 6-seed 변동폭
(§8.1 참고: LVIS_AP 폭 0.0025, LVIS_flip 폭 0.35%p)보다 작은 차이라
유의미한 개선이라 보기 어렵다.

**BRECQ 대비도 3-seed 때보다 더 나빠짐**:
- Top1_flip: seed0(0.69%)만 이기고 **나머지 5개 seed 전부 BRECQ(0.71%)에
  짐**(0.74~0.80%) — "처음으로 BRECQ를 이겼다"던 §8.8의 발견은 seed0
  한정이었고 재현 안 됨.
- `lost`: **6개 seed 전부** BRECQ(327)보다 나쁨(353~397) — 여전히
  미해결이고, 오히려 확정값(372)보다도 근소하게 나쁨.
- LVIS_flip/LVIS_lost는 seed1(LVIS_lost 1201)을 빼면 대체로 BRECQ보다
  낫지만, 확정값과 비교하면 우열이 뚜렷하지 않음.

**유일하게 견고한 개선 — APr(rare class)**: 6개 seed 전부 0.0294~0.0407
범위로, 확정값의 유일한 참고값(seed0=0.0270, §9)보다 전부 높다. 이건
seed 전반에서 일관되게 나타나서 진짜 개선으로 보이는 유일한 지표.

**최종 판단**: `nw=0.25, rw=20`으로 확정 하이퍼파라미터를 바꾸지 않는다
— 대부분 지표에서 확정값과 동등하거나 근소하게 못하고, COCO_AP는
확실히 손해다. **APr(rare) 개선만은 seed 전반에서 견고해 보이므로,
논문에는 "이 방향으로 조정하면 rare-class 성능을 개선할 여지가 있다"는
부수적 관찰/어블레이션으로 남겨두되, 채택은 하지 않는다.** 이 탐색
경로(nw↓+rw↑ 조합)는 여기서 종료 — 더 세밀한 탐색(rw=15,25 등)은
기대 이득이 크지 않아 진행하지 않기로 함.

### 8.10 `cal_idx`/`cal_weight` — H_cal 직접 보호 확장, **채택(cal_weight=1.0), §8.1로 승격됨** (09-12)

**동기**: §9에서 짚은 "Combined가 APr(rare)에서 5개 중 최하위" 문제의
메커니즘 가설은 "margin_loss가 S(40개)만 직접 최적화하고, H_cal(20개)은
S의 neighbor로 우연히 뽑혀야만 asymmetric hinge로 간접·수동적으로만
보호받는다"는 것. 최적화 압력이 S에만 집중되면서 COCO-80 전반의 `lost`/
APr이 희생된다는 정황(§9)에 대응해, **H_cal에도 S와 동일한 margin_loss를
직접 적용**해서 보호 대상을 S∪H_cal(60개, COCO-80의 75%)로 넓히는 실험을
했다.

**구현**: `optimize_promptcal_scale_neighbor`에 `cal_idx`/`cal_weight`
파라미터 추가. `cal_idx`(H_cal 인덱스)가 주어지면 `margin_loss(sim_q[:,S],
sim_fp[:,S])`와 동일한 함수를 H_cal 컬럼에도 적용해 `cal_weight`로 가중해
더한다. `cal_idx=None`(기본)이면 기존 동작과 완전히 동일(opt-in).
`src/quant/promptcal.py`, `pipeline/quant/promptcal.py`(동기화),
`scripts/58_full_baseline_official_data.py`(`--cal-weight`),
`scripts/60_hparam_sweep_official_data.py`(`--param cal_weight`)에 반영.

**버그 발견 및 수정(구현 중)**: 초안은 "이미 margin_loss로 직접 보호하니
neighbor-hinge 후보 풀에서도 cal_idx를 제외해 이중 처리를 막자"는
의도로 짰는데, **S(40)+H_cal(20)+H_eval(20) = 정확히 COCO-80 전체(80개)**
라서 이 셋을 전부 제외하면 neighbor 후보가 0개가 되어 `neighbor_cols`가
항상 빈 텐서였다. `F.mse_loss`/`F.relu(...).pow(2).mean()`이 빈 텐서에
대해 조용히 `nan`을 반환하는데, 빈 인덱싱의 backward는 실제로는 0
gradient만 주기 때문에 `s_mult`가 nan으로 오염되지는 않고 **neighbor-hinge
항 자체가 통째로 무효화된 채 조용히 실행**되는 형태로 나타났다(재현
스크립트로 `neighbor 0개`/`neighbor_loss=nan` 확인). 이 상태로 첫 스모크
테스트와 seed0 1차 스윕을 돌려서, 처음 관찰한 "극적인 APr 개선"은 사실
"H_cal 보호 추가"가 아니라 **"neighbor-hinge가 꺼진 상태에서의 순수
margin_loss(S∪H_cal) 학습"**을 측정한 것이었다. `cal_idx`를 neighbor
후보 풀 제외 대상에서 뺘서 수정 — 이제 `cal_weight=0`이 기존 확정값과
비트 단위로 일치함을 확인(불변식 검증 완료).

**OFAT 스윕(수정 후, seed0)**:

| cal_weight | COCO_AP | LVIS_AP | APr | APc | UPIR | lost |
|---|---|---|---|---|---|---|
| 0(확정) | 36.38 | 0.1246 | 0.0270 | 0.0789 | 0.30% | 369 |
| 0.5 | 36.30 | 0.1222 | 0.0407 | 0.0730 | 0.28% | 368 |
| 1.0 | 36.34 | 0.1241 | 0.0367 | 0.0776 | 0.26% | 358 |
| 2.0 | 36.26 | 0.1240 | 0.0303 | 0.0798 | 0.22% | 331 |
| 4.0 | 36.41 | 0.1237 | 0.0377 | 0.0766 | 0.27% | 350 |

**6-seed(0~5) 검증**(`cal_weight=1.0`·`4.0` vs 확정값, `runs/60_hparam_sweep/cal_weight_seed{0..5}.log`):

| method | COCO_AP | LVIS_AP | APr | Heval_flip | Top1_flip | UPIR | lost | LVIS_flip | LVIS_lost |
|---|---|---|---|---|---|---|---|---|---|
| 확정값(cal_weight=0) | 36.19~36.48(36.30) | 0.1229~0.1254(0.1240) | 0.0269~0.0471(0.0360) | 7.76~10.52%(9.39%) | 0.68~0.76%(0.727%) | 0.16~0.31%(0.243%) | 358~376(367.5) | 5.15~5.49%(5.33%) | 1076~1151(1114) |
| **cal_weight=1.0** | 36.29~36.40(36.36) | 0.1213~0.1260(0.1238) | 0.0320~0.0429(**0.0371**) | 7.87~11.04%(9.52%) | 0.70~0.75%(0.727%) | 0.11~0.29%(**0.212%**) | 336~391(362.7) | 5.17~5.42%(5.32%) | 1021~1124(**1083**) |
| cal_weight=4.0 | 36.31~36.51(36.41) | 0.1226~0.1251(0.1240) | 0.0280~0.0475(0.0384) | 7.62~10.71%(9.45%) | 0.71~0.77%(0.732%) | 0.14~0.30%(0.217%) | 350~396(366.2) | - | - |

**정직한 요약**:
- seed0 단독으로 봤던 "APr +36~51%"는 과장이었다 — APr 자체가 확정값
  기준으로도 seed마다 0.0269~0.0471로 크게 흔들리는 지표라서. 6-seed
  평균으로는 cal_weight=1.0이 +3%(0.0371 vs 0.0360), 4.0이 +7%(0.0384)
  개선. 다만 6개 중 5개 seed에서 개별적으로도 확정값을 이겨서 방향성
  자체는 일관됨.
- COCO_AP/LVIS_AP는 cal_weight=1.0·4.0 모두 확정값과 사실상 동등(평균
  차이가 6-seed 변동폭보다 작음).
- **UPIR/LVIS_lost는 cal_weight=1.0에서 뚜렷이 개선**(UPIR 0.243%→0.212%,
  LVIS_lost 1114→1083) — 이 두 지표는 확정값도 BRECQ를 못 이기던
  지표라 격차를 좁히는 방향.
- `lost`(COCO-80)만 여전히 BRECQ(327)를 못 넘음 — 유일하게 안 풀리는
  지표는 그대로.
- Top1_flip은 평균이 완전히 동률(0.727%)이라 사실상 노이즈 수준 차이.
- `nw=0.25,rw=20`(§8.9, 기각)과 달리 **뚜렷하게 나빠지는 지표가 없다** —
  "손해 없이 대부분 지표가 같거나 소폭 낫다"는 패턴.

**cal_weight=1.0 vs 4.0 선택**: 4.0이 COCO_AP/APr에서 근소히 앞서지만
격차가 6-seed 변동폭 안이고, 1.0이 UPIR/lost(둘 다 BRECQ와 격차가
가장 큰 지표)에서 더 낫다. 또한 1.0은 `neighbor_weight`의 기본값과
같아서 "S를 지키는 것과 동일한 가중치로 H_cal도 직접 지킨다"는 대칭적
설명이 가능해 논문 스토리상 더 자연스럽다. **`cal_weight=1.0`을 최종
채택값으로 확정.**

*원본: `runs/60_hparam_sweep/cal_weight_seed0_fixed.log`(seed0 OFAT),
`runs/60_hparam_sweep/cal_weight_seed{0,1,2,3,4,5}.log`(6-seed 검증).
버그 있던 최초 결과(`cal_weight_seed0.log`, 스모크 `smoke_calweight.log`)는
neighbor-hinge가 무효화된 상태라 참고하지 말 것.*
...
- **APr(rare class)에서 Combined가 5개 중 최하위 — 잠재적으로 심각한 문제(09-10 발견)**:
  ...
  **부분 완화(09-12, §8.10)**: H_cal에도 margin_loss를 직접 적용하는
  `cal_weight=1.0`을 채택해서 6-seed 평균 APr이 0.0360→0.0371(+3%)로
  개선(6개 중 5개 seed에서 개별적으로도 이김). 다만 여전히 naive
  (0.0415)보다는 낮아서 "완전 해소"는 아니고, 개선폭 자체도 크지 않음 —
  완화됐을 뿐 근본적으로 풀린 문제는 아니라고 보는 게 정확함.
- **UPIR·lost가 최선이 아님**: BRECQ가 이 두 지표는 더 낮음(=더 좋음), 특히
  ...
  **결론: COCO-80-국소적 비용이라는 가설은 기각. `lost` 악화는 vocabulary
  ...
  이 표는 `cal_weight` 도입 **전**(neighbor-hinge만으로 H_cal을 간접
  보호하던 시점)의 관측이라 위 수치 자체는 지금은 §8.1 기준으로
  달라졌지만, "neighbor-hinge가 보호하는 그룹(H_cal)이 상대적으로 덜
  나빠진다"는 이 관찰이 바로 §8.10에서 H_cal을 아예 직접 최적화 대상으로
  넣어보자는 아이디어(`cal_weight`)로 이어졌고, 실제로 `lost` 평균이
  372→362.7로 개선됨.
...
- ~~**H_cal(20개) 미사용**: 3분할 중 H_cal은 현재 Combined 학습에서 전혀
  안 쓰인다~~ — **해결됨(09-12, §8.10)**: `cal_weight=1.0` 채택으로
  H_cal에도 S와 동일한 margin_loss를 직접 적용하고, 여전히
  neighbor-hinge 후보 풀(H_cal 20개가 사실상 유일한 후보 -- S/H_eval
  제외하면 이것만 남음) 역할도 겸한다. 즉 H_cal은 이제 "직접 최적화
  대상"이자 "neighbor 보호 대상" 둘 다로 쓰인다. 구현 중 cal_idx를
  neighbor 후보 풀에서도 제외하려던 초안이 S+H_cal+H_eval=80(COCO-80
  전체)이라 후보 풀을 통째로 비워 neighbor-hinge를 무효화하는 버그였음
  — 자세한 경위는 §8.10 참고.
