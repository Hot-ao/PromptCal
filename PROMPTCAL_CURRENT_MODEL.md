...
### 8.1 메인 결과 — `scale_reg_weight=10`(확정, §8.2 판단 근거), **6-seed(0~5) 완료**

rw=10/rw=20 둘 다 6-seed(0,1,2,3,4,5) 전부 완료(seed3는 최초 OOM으로
소실됐다가 메모리 버그 수정 후 재실행). **범위는 평균±표준편차가 아니라
최소~최대**로 표시(괄호 안이 평균). baseline은 AP/lost/LVIS류는 seed와
무관한 상수, Heval_flip·UPIR만 S/H_eval 분할 때문에 seed마다 달라서 같이
범위로 표시했다. **GT_MRR/GT_R@1**(실제 COCO GT-anchor에서 quant 모델 자신의
랭킹 역수 평균 / 1등 비율, §7 참고)도 추가했다 — baseline은 S/H_eval 분할과
무관하게 계산돼서(UPIR과 달리 H_eval_set을 안 씀) seed에 상관없이 완전히
동일한 상수, Combined만 S가 seed마다 바뀌어 학습 자체가 달라지니 소폭
범위를 가진다.
...
| **Combined(rw10)** | **36.19~36.45(36.34)** | 0.1213~0.1272(0.1253) | **8.05~10.50%(9.47%)** | 0.68~0.78%(0.75%) | 0.9221~0.9231(0.9226) | 0.8764~0.8782(0.8773) | 0.10~0.31%(0.23%) | 352~390(371.3) | **5.20~5.56%(5.35%)** | **1052~1159(1093.8)** |

*원본 로그: `runs/58_official_data/{seed0,seed1,seed2,seed4}.log`(baseline·rw20 참고용),
`runs/58_official_data/rw10_seed{1,2,3,4,5}.log` + `runs/59_rw_sweep/rw_sweep_seed0.log`(rw10 seed0)*

**GT_MRR/GT_R@1 대조가 보여주는 것(중요)**: 5개 방법의 GT_MRR(0.9220~0.9233)·
GT_R@1(0.8764~0.8788)이 **거의 완전히 겹친다** — 이 절대적 랭킹 정확도만
보면 naive조차 다른 방법들과 별 차이가 없다. 그런데 같은 5개 방법의 AP는
33.24~36.45로 크게 갈리고, Top1_flip/Heval_flip도 뚜렷이 갈린다. **이
괴리 자체가 핵심 논증이다**: AP 개선이 "GT 랭킹 점수 자체를 절대적으로 더 잘
복원해서" 나온 게 아니라(그랬다면 GT_MRR도 같이 갈렸어야 함), **"FP32가
매기던 순서/결정을 얼마나 유지하는가"(flip)에서 나온다**는 뜻 — 이는
`margin_loss`가 값(score) 자체가 아니라 순위 간격(margin)을 맞추도록
설계된 것과 정확히 같은 철학이다.

**정직한 요약**:
- COCO_AP: naive 대비 **+2.80**(평균 기준), 6-seed 범위(36.19~36.45)가
  naive(33.54)와 전혀 안 겹칠 만큼 안정적으로 높음.
- Heval_flip: 평균 9.47%로 BRECQ 평균(10.48%)보다 낮음(더 좋음) — masked
  flip을 이긴 건 이 설계가 처음(§5.2 재설계 이전엔 항상 최악~근접 최악).
  다만 범위가 넓어져서(8.05~10.50%) BRECQ의 최선 seed(8.55%)와는 겹치는
  구간이 있음 — "항상" 이긴다기보다 "평균적으로/대체로" 이긴다가 정확함.
- LVIS_flip: 평균 5.35%로 BRECQ(5.53%)보다 낮음, 6개 중 5개 seed가 개별적으로도 낮음.
- LVIS_lost: 평균 1093.8로 BRECQ(1158)보다 확실히 낮음(6개 중 5개 seed도 개별적으로 낮음).
- **LVIS_AP**: 평균 0.1253으로 naive(0.1264)와 거의 붙음(차이 -0.9% 상대,
  이전 rw20 6-seed의 -1.3%보다 격차가 줄어듦) — seed5는 오히려 naive/FP32를
  넘어섬(0.1272). AdaRound/QDrop/BRECQ보다는 6개 seed 전부 명확히 위.
