"""
D-3: held-out 조건에서 naive vs QDrop 비교 (AdaRound 5.24% 대비).
...
실행:
    CUDA_VISIBLE_DEVICES=0 python scripts/10_qdrop.py \
...
def build_quant(model_cls, weights, names, device, calib_tensors, adaround=False,
                fp_module=None, iters=2000, qdrop_prob=0.0):
...
def main():
    ...
    ap.add_argument("--qdrop-prob", type=float, default=0.5, help="QDrop activation drop 확률")
    ...
    print("[build] QDrop-quant (activation drop, 최적화 ~12min)")
    t0=time.time()
    ...
    print("\n"+"="*72)
    print(" HELD-OUT: naive vs QDrop (AdaRound held-out=5.24% 참고)")
    print("="*72)
    print(f"{'seed':>5} | {'target':>7} | {'seen_nv':>7} | {'seen_qd':>7} | "
          f"{'ho_nv':>6} | {'ho_qd':>6} | {'ho 감소':>7}")
    print("-"*72)
    hn_l,ha_l=[],[]
    for s in args.seeds:
        a=acc[s]; n=max(a["n"],1)
    ...
    print("="*72)
    print("\n해석:")
    print(" - QDrop ho가 naive/AdaRound와 비슷하면 → activation 강건화로도 held-out 못 지킴 = 논지 강력 보강.")
    print("   (AdaRound held-out 5.24% 대비 QDrop이 더 낮추면 위협, 비슷/높으면 보강)")
    print(" - weight 축(AdaRound)과 activation 축(QDrop) 모두 held-out 실패 시, reconstruction 계열 전체가 실패.")
