"""
k x neighbor_k x boundary_w 조합 그리드 확인 (09-10, H_eval 리크 수정 후).

scripts/60의 1-seed OFAT 재스윕 결과, neighbor_k>=5는 전부 동일(neighbor 후보
풀 H_cal 20개가 neighbor_k=5에서 이미 포화)이라 neighbor_k는 사실상 5로
고정해도 된다. k는 top 2(5, 8), boundary_w는 top 2(재스윕 결과로 결정)로
그리드를 좁혀서 상호작용(negative interaction 가능성, scripts/60의 k=8+
boundary_w=5 조합 테스트에서 실제로 관찰된 적 있음)을 확인한다.

방법론: OFAT(scripts/60)로 각 파라미터의 top 2 후보를 먼저 뽑고, 그 후보들의
전체 조합(cartesian product)만 확인한다 -- 전체 그리드(96개)는 비현실적이라
후보를 좁힌 것. scripts/60과 마찬가지로 naive/AdaRound/QDrop/BRECQ는 다시
안 빌드하고 BASELINE_REF를 쓰고, AdaRound base는 1회만 빌드해서 모든 조합에
deepcopy로 재사용한다. probe는 스트리밍으로 읽는다(scripts/60과 동일,
09-10 메모리 수정 반영).

실행 예:
    CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=4 python scripts/61_combo_grid_official_data.py \
        ...
        --k-values 5,8 --boundary-w-values 3,5 --neighbor-k-values 5 --seed 0 --device 0
"""
import argparse, copy, glob, itertools, os, sys, time
...
def main():
    ...
    ap.add_argument("--k-values", default="5", help="콤마 구분 정수 리스트")
    ap.add_argument("--boundary-w-values", default="3.0", help="콤마 구분 실수 리스트")
    ...
    nk_values = [int(x) for x in args.neighbor_k_values.split(",")]
    grid = list(itertools.product(k_values, nk_values, bw_values))
    print(f"[grid] k={k_values} x neighbor_k={nk_values} x boundary_w={bw_values} "
          f"-> {len(grid)}개 조합")
    ...
    # probe는 스트리밍으로 읽는다 (scripts/58/59/60과 동일, 09-10 메모리 수정)
    ...
    conditions = [f"combined_k{k:g}_nk{nk:g}_bw{bw:g}" for (k, nk, bw) in grid]
    models, calib_time = {}, {}
    for (k, nk, bw), mode in zip(grid, conditions):
        print(f"[build] {mode} (k={k}, neighbor_k={nk}, boundary_w={bw})")
        t0 = time.perf_counter()
    ...
    print("\n" + "=" * 100)
    print(f" k x neighbor_k x boundary_w 조합 그리드 (scale_reg_weight={args.scale_reg_weight}) "
          f"(calib=train2017 {len(calib_paths)}장, LVIS=공식 minival) -- seed {args.seed}")
    print("=" * 100)
    print(f"{'method':>28} | {'COCO_AP':>8} | {'LVIS_AP':>8} | {'Heval_flip':>10} | "
          f"{'Top1_flip':>9} | {'UPIR':>6} | {'lost':>5} | {'LVIS_flip':>9} | {'LVIS_lost':>9}")
    for name, r in BASELINE_REF.items():
        print(f"{name:>28} | {r['coco_ap']:>8.2f} | {r['lvis_ap']:>8.4f} | {r['heval_flip']:>9.2f}% | "
              f"{r['top1_flip']:>8.2f}% | {r['upir']:>5.2f}% | {r['lost']:>5} | "
              f"{r['lvis_flip']:>8.2f}% | {r['lvis_lost']:>9}")
    for mode in conditions:
        r = results[mode]
        print(f"{mode:>28} | {r['coco_ap']:>8.2f} | {r['lvis_ap']:>8.4f} | {r['heval_flip']:>9.2f}% | "
